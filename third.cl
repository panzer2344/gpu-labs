...
__kernel void matmul_block_image( const int rows,  const int cols,  const int size, __read_only image2d_t A,  __read_only image2d_t B, _global image2d_t C) {
    const int row = get_local_id(0);
    const int col = get_local_id(1);
    ...
    for (int t = 0; t < num_tiles; t++) {
		 ...
		 //printf("A_sub[%d][%d]=%f", col, row, A_sub[col][row]);
		 //printf("B_sub[%d][%d]=%f", col, row, B_sub[col][row]);
	...
	//printf("a_sub[%d][%d]=%f", col, row, C[][row]);
	//printf("acc = %f \n", acc);
