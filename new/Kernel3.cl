...
__kernel void matmul_block_image( const int rows,  const int cols,  const int size, __read_only image2d_t A,  __read_only image2d_t B, __write_only image2d_t C) {
    ...
    for (int t = 0; t < num_tiles; t++) {
		...
		A_sub[col][row] = read_imagef(A, id_A).x;
		B_sub[col][row] = read_imagef(B, id_B).x;
		barrier(CLK_LOCAL_MEM_FENCE);
		...
		//printf("A[%d][%d]=%f\n", global_row, global_col, A_sub[global_row][global_col]);
		//printf("B[%d][%d]=%f\n", global_row, global_col, B_sub[global_row][global_col]);

		//printf("C[%d][%d]=%f\n", global_row, global_col, acc);
